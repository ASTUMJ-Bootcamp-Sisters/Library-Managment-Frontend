...
const BookDetail = () => {
  ...
  const [book, setBook] = useState({ comments: [], averageRating: 0 }); // default values
  ...
  const fetchBook = async () => {
    try {
      ...
      // Optional: set current user's rating
  ...
  {book.comments.length > 0 ? (
    book.comments.map((c) => (
      <div
        key={c._id}
        className="border-b py-2 flex gap-2 items-start"
      >
