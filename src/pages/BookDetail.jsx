...
const BookDetail = () => {
  ...
  const checkMembershipStatus = async () => {
    try {
      setUserMembership(null); // simulate non-member
    } catch (err) {
      console.error(err);
      setUserMembership(null);
    }
  ...
  const fetchBook = useCallback(async () => {
    try {
      ...
      const currentUserComment = bookData.comments?.find(c => c.user?._id === currentUserId);
      if (currentUserComment) setUserRating(currentUserComment.rating || 0);
      ...
      if (favRes.data.favourites?.some(fav => fav._id === id)) setIsFavourite(true);
  ...
  const handleBorrowDialogOpen = () => {
    if (book.available <= 0) {
      toast({ title: "Book unavailable", description: "Out of stock", variant: "destructive" });
      return;
    }
    setBorrowDialogOpen(true);
  };
  ...
  const handleBorrowSubmit = async (e) => {
    ...
    if (!isMember) {
      if (!idCardImage || !paymentImage) {
        toast({ title: "Missing Files", description: "Upload ID and Payment proof", variant: "destructive" });
        return;
      }
    }
    ...
    try {
      ...
      toast({ title: "Success", description: response.isMember ? "Book borrowed" : "Borrow request sent" });
    } catch (err) {
      console.error(err);
      toast({ title: "Error", description: "Failed to borrow", variant: "destructive" });
  ...
  const toggleFavourite = async () => {
    try {
      if (isFavourite) {
        await axios.post("/api/favourites/remove", { bookId: id }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
        setIsFavourite(false);
        toast({ title: "Removed", description: "Book removed from favourites" });
      } else {
        await axios.post("/api/favourites/add", { bookId: id }, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
        setIsFavourite(true);
        toast({ title: "Added", description: "Book added to favourites ❤️" });
      }
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-[#fffaf3] p-6">
      ...
      <Dialog open={borrowDialogOpen} onOpenChange={setBorrowDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Borrow Book</DialogTitle>
            <DialogDescription>
              {userMembership?.status === "Active" ? "Complete the form to borrow." : "Upload ID & Payment proof."}
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleBorrowSubmit} className="space-y-4">
            ...
            {userMembership?.status !== "Active" && <>
              ...
              <div className="space-y-2">
                <Label htmlFor="payment">Payment Proof (Required)</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="note">Notes (Optional)</Label>
              <Textarea id="note" value={borrowNote} onChange={(e) => setBorrowNote(e.target.value)} placeholder="Any notes..." className="w-full" />
            </div>

            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setBorrowDialogOpen(false)}>Cancel</Button>
              <Button type="submit" disabled={isSubmitting} className="bg-[#4a2c1a] hover:bg-[#633b25]">{isSubmitting ? "Processing..." : "Borrow"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-lg p-6 flex flex-col md:flex-row gap-6">
        ...
        <div className="md:w-2/3 flex flex-col gap-3">
          <h1 className="text-4xl font-extrabold text-[#4a2c1a]">{book.title}</h1>
          <p className="text-lg text-gray-700"><strong>Author:</strong> {book.author}</p>
          <p className="text-lg text-gray-700"><strong>Category:</strong> {book.category?.name || "N/A"}</p>
          <p className="text-lg text-gray-700"><strong>Language:</strong> {book.language || "N/A"}</p>
          <p className="text-lg text-gray-700"><strong>Publisher:</strong> {book.publisher || "N/A"}</p>
          <p className="text-lg text-gray-700"><strong>Year:</strong> {book.year || "N/A"}</p>
          <p className="text-lg text-gray-700"><strong>ISBN:</strong> {book.isbn || "N/A"}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-4">
            <Button className="bg-[#4a2c1a] hover:bg-[#633b25]" onClick={handleBorrowDialogOpen} disabled={book.available <= 0}>
              {book.available > 0 ? "Borrow this book" : "Currently unavailable"}
            </Button>

            <Button variant="outline" onClick={toggleFavourite} className={`flex items-center gap-2 ${isFavourite ? "text-red-600 border-red-600" : "text-gray-700"}`}>
              <Heart className={`w-5 h-5 ${isFavourite ? "fill-red-600" : ""}`} />
      ...
      <div className="max-w-6xl mx-auto mt-6 p-6 bg-white shadow-lg rounded-lg">
        ...
        <h2 className="text-2xl font-bold text-[#4a2c1a] mb-4">Comments</h2>
        {book.comments.length > 0 ? book.comments.map(c => (
          <div key={c._id} className="border-b py-3 flex gap-3 items-start">
            <Avatar className="bg-slate-400">
              {c.user?.profileImage ? <AvatarImage src={c.user.profileImage} /> : <AvatarFallback>{c.user?.fullName?.split(" ").map(n=>n[0]).join("") || "U"}</AvatarFallback>}
            </Avatar>
            <div className="flex-1">
              <p className="font-semibold text-gray-800">{c.user?.fullName || "User"}</p>
              <p className="text-gray-700">{c.text}</p>
